...
async function requireStaff() { const session = await getServerSession(authOptions); if (!isStaff(session?.user?.role)) throw new Error("Недостаточно прав"); return session; }
...
export async function uploadResultAction(formData: FormData) {
  ...
  await prisma.designResult.create({ data: { orderId, imageUrl: stored.fileUrl, title: String(formData.get("title") || file.name), description: String(formData.get("description") || "Готовый вариант дизайна"), isPaid: true } });
  await prisma.orderFile.create({ data: { orderId, uploadedBy: session.user!.id, ...stored } });
